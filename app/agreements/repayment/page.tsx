...
export default function RepaymentPage() {
    ...
    // Get all loans flattened
    const allLoans = repaymentData.flatMap(c => c.loans);
    ...
    // Group loans by group_no
    const groupedLoans: GroupedLoan[] = React.useMemo(() => {
        const groups: Record<string, RepaymentLoan[]> = {};
        filteredLoans.forEach(loan => {
            if (!groups[loan.group_no]) {
                groups[loan.group_no] = [];
            }
            groups[loan.group_no].push(loan);
        });
    ...
    const pageTotals = React.useMemo(() => {
        return {
            loan_amount: filteredLoans.reduce((sum, l) => sum + (l.loan_amount || 0), 0),
            balance_amount: filteredLoans.reduce((sum, l) => sum + (l.balance_amount || 0), 0),
            rental: filteredLoans.reduce((sum, l) => sum + (l.rental || 0), 0),
            arrears: filteredLoans.reduce((sum, l) => sum + (l.arrears || 0), 0),
        };
    }, [filteredLoans]);
            ...
            <div className="filter-bar bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 p-4 md:p-5">
                <div className="flex flex-col lg:flex-row lg:items-end gap-4">
                    ...
                    <div className="flex gap-2 mt-5 lg:mt-0">
                        <button onClick={handleExportCSV} disabled={filteredLoans.length === 0} className="flex items-center gap-2 px-4 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all shadow-lg shadow-emerald-500/20 font-bold text-sm disabled:opacity-50 whitespace-nowrap">
                            <Download className="w-4 h-4" /> CSV
                        </button>
                        <button onClick={handlePrint} disabled={filteredLoans.length === 0} className="flex items-center gap-2 px-6 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl transition-all shadow-lg shadow-blue-500/20 font-bold text-sm disabled:opacity-50 whitespace-nowrap">
            ...
            <div ref={printRef} className="print-area">
                {loading ? (
                    ...
                    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-200 dark:border-gray-700 overflow-hidden">
                        <div className="document-header text-center py-4 px-6 border-b border-gray-200 dark:border-gray-700">
                            <h2 className="text-base font-bold text-gray-900 dark:text-white uppercase tracking-wide">Attendance / Repayment</h2>
                            <p className="text-[10px] text-gray-600 dark:text-gray-400 mt-1 font-medium">
                                CSU Name: <span className="font-bold">{selectedCenterInfo?.center_name || "—"}</span>
                                {" "}CSU Code: <span className="font-bold">{selectedCenterInfo?.csu_id || "—"}</span>
                                {" "}Print User: <span className="font-bold uppercase">{selectedStaffName}</span>
                                {" "}Print Date: <span className="font-bold">{formatDisplayDate(new Date().toISOString())}</span>
                            </p>
                        </div>
                        ...
                        <div className="w-full">
                            <table className="w-full border-collapse repayment-table text-[10px]">
                                <thead>
                                    <tr className="bg-gray-50 dark:bg-gray-700/40 print:bg-gray-100">
                                        ...
                                        <th className="repayment-th p-0 w-[65px] min-w-[65px]">
                                            ...
                                            <div className="flex divide-x divide-gray-300 dark:divide-gray-600 h-5">
                                ...
                                <tfoot>
                                    <tr className="bg-gray-50 dark:bg-gray-700/30 font-black">
                                        <td colSpan={5} className="repayment-td text-center uppercase text-[10px] tracking-[0.2em] py-3">Page Total</td>
                                        <td className="repayment-td text-right">{formatNumber(pageTotals.loan_amount)}</td>
                                        <td className="repayment-td text-right">{formatNumber(pageTotals.balance_amount)}</td>
                                        <td className="repayment-td text-right">{formatNumber(pageTotals.rental)}</td>
                                        <td className="repayment-td text-right">{formatNumber(pageTotals.arrears)}</td>
