    ...
    if (loading) {
        return (
            <div className="min-h-screen flex flex-col items-center justify-center p-20 bg-app-background">
                <BMSLoader message="Analyzing Portfolio..." size="small" />
            </div>
        );
    }
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
                    ...
                    <div className="lg:col-span-12 xl:col-span-4 space-y-8">
                        <div className="bg-card/80 backdrop-blur-xl rounded-[2.5rem] border border-border-default shadow-xl overflow-hidden group">
                            <div className="bg-gradient-to-br from-indigo-600 to-indigo-900 p-8 text-white relative">
                                ...
                                <div className="relative z-10">
                                    <p className="text-[9px] font-black uppercase tracking-[0.3em] opacity-60 mb-1">Applicant Reference</p>
                            ...
                            <div className="p-8 space-y-6">
                                {[
                                    { label: 'NIC Identifier', value: staffDetails?.nic, icon: ShieldCheck },
                                    { label: 'Sector Location', value: staffDetails?.branch, icon: Landmark },
                                    { label: 'Comms Link', value: staffDetails?.contact_no, icon: Sparkles },
                                    { label: 'Tactical Residence', value: staffDetails?.address, icon: Hash, full: true }
                                ].map((detail, idx) => (
                                    <div key={idx} className={`space-y-1.5 ${detail.full ? 'border-t border-border-divider pt-4 mt-2' : ''}`}>
                                        <p className="text-[9px] font-black text-text-muted uppercase tracking-widest flex items-center gap-2">
                                            <detail.icon size={12} className="text-indigo-400" /> {detail.label}
                                        </p>
                        ...
                        <div className="bg-indigo-600 rounded-[2rem] p-8 text-white shadow-xl relative overflow-hidden group">
                            ...
                            <div className="relative z-10">
                                <h3 className="text-lg font-black uppercase tracking-tight mb-2">Notice of protocol</h3>
                                <p className="text-[11px] font-bold text-indigo-100 leading-relaxed uppercase opacity-80">
                                    Every submission initiates a high-security validation protocol across the central administrative ledger.
                                </p>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="lg:col-span-12 xl:col-span-8 bg-card/80 backdrop-blur-xl rounded-[2.5rem] border border-border-default shadow-xl p-10">
                        <div className="flex items-center gap-4 mb-10">
                            ...
                            <div>
                                <h2 className="text-xl font-black text-text-primary uppercase tracking-tight leading-none mb-1">Entry Terminal</h2>
                                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Protocol Generation Interface</p>
                            </div>
                        </div>
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                                <div className="space-y-3">
                                    <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Asset Portfolio *</label>
                                ...
                                <div className="space-y-3">
                                    <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Verified Witness *</label>
                                    <div className="relative">
                                ...
                                <div className="space-y-3">
                                    <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Portal Valuation (LKR) *</label>
                                    <div className="relative">
                                ...
                                <div className="space-y-3">
                                    <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Duration Protocol (Months) *</label>
                                    <div className="relative">
                                ...
                                <div className="col-span-1 md:col-span-2 space-y-3">
                                    <label className="block text-[10px] font-black text-text-muted uppercase tracking-widest ml-1">Mission Rationale *</label>
                                    <textarea
                                        required
                                        value={formData.purpose}
                                        onChange={e => setFormData({ ...formData, purpose: e.target.value })}
                                        placeholder="Document rationale for asset acquisition..."
                            ...
                            <button
                                type="submit"
                                disabled={submitting}
                                className="w-full py-5 bg-gray-900 text-white rounded-2xl text-[11px] font-black uppercase tracking-[0.2em] shadow-xl hover:bg-black transition-all active:scale-[0.98] disabled:opacity-50 flex items-center justify-center gap-3 group/btn relative overflow-hidden"
                            >
                                <div className="absolute inset-0 bg-gradient-to-r from-primary-600 to-indigo-600 opacity-0 group-hover/btn:opacity-100 transition-opacity" />
                    ...
                    <div className="lg:col-span-12 bg-card/70 backdrop-blur-xl rounded-[2.5rem] border border-border-default shadow-xl overflow-hidden mt-8">
                        <div className="p-8 border-b border-border-divider flex items-center justify-between">
                            <div>
                                <h3 className="text-xl font-black text-text-primary uppercase tracking-tight mb-1">Entry Ledger</h3>
                                <p className="text-[10px] font-black text-text-muted uppercase tracking-widest">Historical Application Sequence</p>
                            </div>
                            ...
                            <table className="w-full text-left">
                                <thead className="bg-table-header">
                                    <tr className="border-b border-border-default">
                                        <th className="pl-10 pr-4 py-6 text-[9px] font-black text-text-muted uppercase tracking-widest">Sequence Date</th>
                                        <th className="px-4 py-6 text-[9px] font-black text-text-muted uppercase tracking-widest">Protocol Rationale</th>
                                        <th className="px-4 py-6 text-[9px] font-black text-text-muted uppercase tracking-widest">Valuation</th>
                                        <th className="px-4 py-6 text-[9px] font-black text-text-muted uppercase tracking-widest">Duration</th>
                                        <th className="px-4 py-6 text-[9px] font-black text-text-muted uppercase tracking-widest">Witness</th>
                                        <th className="pr-10 pl-4 py-6 text-right text-[9px] font-black text-text-muted uppercase tracking-widest">Status Board</th>
                                    </tr>
                                </thead>
                                    ...
                                    {myLoans.length === 0 && (
                                        <tr>
                                            <td colSpan={6} className="py-24 text-center opacity-30">
                                                ...
                                                <p className="text-[10px] font-black uppercase tracking-[0.4em]">Historical Ledger Purged</p>
                                            </td>
                                        </tr>
                                    )}
                                </tbody>
