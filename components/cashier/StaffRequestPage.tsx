                    ...
                    <form onSubmit={handleSubmit} className="flex flex-col gap-8">
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                            ...
                            <div className="flex flex-col gap-6">
                                <div className="flex flex-col gap-2 group">
                                    <label className="text-xs font-black text-text-muted uppercase tracking-wider group-focus-within:text-primary-500 transition-colors">Amount (LKR) *</label>
                                    <div className="relative">
                                        <DollarSign size={18} className="absolute left-4 top-1/2 -translate-y-1/2 text-text-muted group-focus-within:text-primary-500 transition-colors" />
                                        <input
                                            type="number"
                                            step="0.01"
                                            value={amount}
                                            ...
                                            placeholder="0.00"
                                            className="w-full pl-12 pr-4 py-4 rounded-2xl bg-app-background border border-border-default text-text-primary text-lg font-black focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all placeholder:text-text-muted/20"
                                        />
                                    </div>
                                </div>

                                <div className="flex flex-col gap-2 group">
                                    <label className="text-xs font-black text-text-muted uppercase tracking-wider group-focus-within:text-primary-500 transition-colors">Reason / Purpose *</label>
                                    <textarea
                                        value={reason}
                                        onChange={(e) => setReason(e.target.value)}
                                        placeholder="Describe why you need these funds..."
                                        rows={4}
                                        className="w-full px-4 py-4 rounded-2xl bg-app-background border border-border-default text-text-primary text-sm font-medium focus:outline-none focus:border-primary-500 focus:ring-4 focus:ring-primary-500/10 transition-all placeholder:text-text-muted/20 resize-none"
                                    />
                                </div>
                            </div>
                        </div>
                        ...
                        <div className="flex justify-end items-center gap-4 pt-4 border-t border-border-default">
                            ...
                            <button
                                ...
                                className="px-10 py-3 rounded-xl bg-primary-600 text-white font-bold text-sm hover:bg-primary-700 transition-all shadow-xl shadow-primary-900/20 disabled:opacity-50 flex items-center gap-2"
                            >
                                ...
                                {loading ? (
                                    ...
                                    requests.map((req) => (
                                        <tr key={req.id} className="border-b border-border-default hover:bg-hover active:bg-hover/50 transition-colors group cursor-default">
                                            ...
                                            <td className="px-8 py-5">
                                                <span className="text-xs font-medium text-text-secondary line-clamp-1 max-w-[200px]">{req.reason}</span>
                                            </td>
