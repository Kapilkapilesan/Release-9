...
const BranchActivityRequestPage: React.FC = () => {
    ...
    const stats = {
        total: activeTab === 'staff-request-iou' ? iouRequests.length : requests.length,
        pending: activeTab === 'staff-request-iou' ? iouRequests.length : requests.filter(r => r.status === 'Pending').length,
    };
            ...
            case 'receipt-cancel-request':
                return (
                    ...
                    <div className="flex flex-col gap-6">
                        ...
                        <div className="bg-card rounded-2xl border border-border-default overflow-hidden text-center py-20 text-text-muted">
                            ...
                            <table className="w-full text-left border-collapse">
                                ...
                                <tbody>
                                    {loading ? (
                                        ...
                                        <tr><td colSpan={6} className="text-center py-10 text-text-muted">No pending IOU requests</td></tr>
                                    ) : (
                                        filteredIous.map((req) => (
                                            <tr key={req.id} className="border-b border-border-default hover:bg-hover transition-colors">
                                                ...
                                                <td className="px-6 py-4 text-xs text-text-muted">{req.reason}</td>
                                                <td className="px-6 py-4">
                                                    <span className="text-[10px] font-black uppercase px-2 py-0.5 rounded-full bg-amber-500/10 text-amber-500">
                                                        {req.status}
                                                    </span>
