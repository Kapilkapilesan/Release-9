...
export function InvestmentAgreementDocument({ investment, witnesses }: Props) {
    ...
    return (
        <div className="agreement-document bg-white text-black p-[1in] font-serif leading-relaxed text-[11pt]" style={{ width: '8.27in', minHeight: '11.69in', margin: '0 auto' }}>
            ...
            <div className="signatures signature-section">
                <h3 className="mb-8">SIGNATURES</h3>
                <p>The parties hereby covenant with each other, for themselves and their heirs, executors, administrators, and assigns, to perform and observe all the covenants herein contained.</p>
                ...
                <div className="witnesses mt-12">
                    <h3 className="text-sm">WITNESSES:</h3>
                    {witnesses.map((w, i) => (
                        <div key={i} className="mb-8 pl-4">
                            <p><strong>{i + 1}. Name:</strong> <span className="field-data underline">{w.name || '__________________________'}</span></p>
                            <p><strong>   NIC No:</strong> <span className="field-data underline">{w.nic || '__________________________'}</span></p>
                            <p><strong>   Address:</strong> <span className="field-data underline">{w.address || '__________________________'}</span></p>
                            <div className="w-1/2 border-b border-black mt-8"></div>
                        </div>
                    ))}
                </div>
            </div>

            <div className="footer text-center mt-20">
                <h3 className="text-gray-900 mb-2">COMPANY DETAILS</h3>
                <p className="text-[10px] text-gray-600 uppercase">
                    Head Office: No. 6, 1st Floor, Main Street, Chankanai.<br />
                    Tel: 0212223556 | Web: www.bmscapital.lk | Email: info@bmscapital.lk
                </p>
            </div>
        </div>
    );
