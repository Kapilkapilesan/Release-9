...
export function InvestmentCreate() {
    ...
    const addWitness = () => {
        setFormData(prev => ({
            ...prev,
            witnesses: [...prev.witnesses, { name: '', nic: '', address: '' }]
        }));
    };
                            ...
                            {formData.witnesses.map((witness, idx) => (
                                <div key={idx} className="bg-gray-50/50 p-6 rounded-2xl border border-white shadow-sm animate-in slide-in-from-bottom-2 duration-300">
                                    <div className="flex items-center justify-between mb-4">
                                        <span className="text-[9px] font-black text-primary-600 uppercase tracking-widest">Witness {idx + 1}</span>
                                        <button type="button" onClick={() => removeWitness(idx)} className="p-2 bg-red-50 text-red-400 hover:text-red-600 transition-colors rounded-lg group">
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        ...
                                        <div>
                                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">NIC No <span className="text-red-400">*</span></label>
                                            <input
                                                type="text"
                                                placeholder="Enter NIC number"
                                                className="w-full px-4 py-2.5 rounded-lg border-white border-[1.5px] text-xs font-bold shadow-sm theme-focus-ring outline-none uppercase"
                                                value={witness.nic}
                                                onChange={e => updateWitness(idx, 'nic', e.target.value)}
                                            />
                                        </div>
                                        <div className="md:col-span-2">
                                            <label className="block text-[8px] font-black text-gray-400 uppercase tracking-widest mb-1.5 ml-1">Permanent Address <span className="text-red-400">*</span></label>
                                            <textarea
                                                placeholder="Enter full permanent address"
                                                rows={2}
                                                className="w-full px-4 py-2.5 rounded-lg border-white border-[1.5px] text-xs font-bold shadow-sm theme-focus-ring outline-none uppercase resize-none"
